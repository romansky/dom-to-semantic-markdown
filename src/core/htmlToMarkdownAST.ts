...
export function htmlToMarkdownAST(element: Element, options?: ConversionOptions, indentLevel: number = 0): SemanticMarkdownAST[] {
  ...
  element.childNodes.forEach((childElement) => {
    ...
    if (overriddenElementProcessing) {
      ...
      const textContent = escapeMarkdownCharacters((childElement as Text).textContent?.trim() ?? '');
      if (textContent && !!(childElement as Text).textContent) {
        debugLog(`Text Node: '${textContent}'`);
        // preserve whitespaces when text childElement is not empty
        result.push({ type: 'text', content: (childElement as Text).textContent?.trim() ?? '' });
      }
      ...
      if (/^h[1-6]$/i.test(elem.tagName)) {
        ...
        if ((elem as HTMLAnchorElement).href.startsWith("data:image")) {
          ...
          // if all children are text nodes
        ...
        const markdownTableRows = tableRows.map(row => {
          const cells = Array.from(row.querySelectorAll('th, td')).map((cell, columnIndex) => {
            return {
              ...
              content: htmlToMarkdownAST(cell as Element, options, indentLevel + 1),
        ...
        if (markdownTableRows.length > 0) {
          ...
          if (hasHeaders) {
            ...
            const headerSeparatorCells = Array.from(tableRows[0].querySelectorAll('th, td'))
              .map(() => ({ type: 'tableCell' as const, content: [{ type: 'text', content: '---' }], colId: undefined }));

            const headerSeparatorRow = {
              type: 'tableRow' as const,
              cells: headerSeparatorCells as { type: 'tableCell'; content: SemanticMarkdownAST[]; colId: string | undefined; }[]
            };
        ...
        elem.querySelectorAll('title')
          .forEach(titleElem => {
            if (node && node.content && node.content.standard) {
              node.content.standard['title'] = escapeMarkdownCharacters(titleElem.textContent ?? '');
            }
          });
        ...
        metaTags.forEach(metaTag => {
          ...
          if (property && property.startsWith('og:') && content) {
            if (options.includeMetaData === 'extended' && node.content.openGraph) {
              node.content.openGraph[property.substring(3)] = content;
            }
          } else if (name && name.startsWith('twitter:') && content) {
            if (options.includeMetaData === 'extended' && node.content.twitter) {
